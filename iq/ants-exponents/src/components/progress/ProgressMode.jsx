...
import { TOPICS, TIER_DEFS, LEVELS } from '../../lib/exponentLevels'
import { getSessions } from '../../lib/sessions'
import { getUser } from '../../lib/users'

const TOTAL = LEVELS.length // 36
...
// The home hub: overall %, a "pick up where you left off" hero, and a per-unit
// 4-tier track. "Play →" and the hero both deep-link into a specific tier via
// onPlay. Mirrors Ants & Fractions / 0 → 1.
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {nextUnit ? (
        ...
        <motion.div
          className="w-full rounded-2xl shadow-lg p-5 mb-6 text-center text-white font-black bg-gradient-to-r from-indigo-400 to-violet-500"
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.04 } }}
        >
          🏆 Every unit mastered — you're an Exponents Master!
        </motion.div>
      )}
      ...
      >
        ...
        <div className="w-full bg-gray-100 rounded-full h-4 overflow-hidden">
          <motion.div
            className="h-4 rounded-full bg-gradient-to-r from-indigo-500 to-violet-500"
            initial={{ width: 0 }}
