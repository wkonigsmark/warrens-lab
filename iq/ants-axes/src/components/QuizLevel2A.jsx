...
export default function QuizLevel2A({ mode = 'master', onBack }) {
  ...
  useEffect(() => {
    ...
    document.addEventListener('keydown', handleKeyDown)
    return () => document.removeEventListener('keydown', handleKeyDown)
  }, [showResults, mentorStep, inputSlope, mentorX1, mentorY1, mentorX2, mentorY2])

  // Generate 3 random, non-duplicate questions with positive whole number slopes
  const questions = useMemo(() => {
    const generateQuestion = () => {
      ...
      do {
        ...
        x1 = Math.floor(Math.random() * 6) // 0-5
        y1 = Math.floor(Math.random() * 6) // 0-5

        // Random run (1-4)
        const dx = 1 + Math.floor(Math.random() * 4)

        // Random slope (1-5)
        slope = 1 + Math.floor(Math.random() * 5)

        // Calculate rise to match slope
        const dy = slope * dx

        x2 = x1 + dx
        y2 = y1 + dy
        attempts++
      } while ((x2 > MAX_VALUE || y2 > MAX_VALUE) && attempts < 20)
  ...
  const handleSubmitAnswer = () => {
    ...
    if (slopeVal < 0 || slopeVal > 12) {
      setError('Slope must be between 0 and 12')
      return
    }
  ...
  const handleMentorStep = () => {
    if (mentorStep === 0) {
      // Info step - just move to Point 1 input
      setMentorStep(1)
      setError('')
