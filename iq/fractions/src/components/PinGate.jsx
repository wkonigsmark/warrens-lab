...
// Gates `children` behind a PIN check for `user`. Guest verifies silently
// (no PIN). Re-checks periodically so a lapsed idle timeout re-prompts
// mid-session, not just on next launch.
export default function PinGate({ user, onCancel, children }) {
  ...
  if (isGuest) return null // silent verify above resolves in a tick
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-rose-100 flex items-center justify-center px-6">
      <form onSubmit={submit} className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-xs text-center">
        ...
        <input
          ...
          className={`w-full text-center text-2xl tracking-widest border-2 rounded-xl py-3 mb-3 outline-none ${
            error ? 'border-red-400' : 'border-gray-200 focus:border-amber-400'
          }`}
        />
