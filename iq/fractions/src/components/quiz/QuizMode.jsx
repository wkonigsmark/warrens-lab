...
import { TOPICS, TIER_DEFS, LEVELS, getLevel } from '../../lib/fractionQuiz'
import { getSessions } from '../../lib/sessions'
import QuizShell from './QuizShell'

// nextLevelTitle from current level to next: just the tier label when staying
// in the same topic (there's always a next tier within-topic here, or none).
...
// A light level picker — pick a topic + tier and play. The rich stats
// dashboard (overall %, milestones, full tier-tracks) lives in its own
// "My Progress" tab, not here — see ProgressMode.jsx. `startLevel` lets
// Progress deep-link straight into a specific tier.
export default function QuizMode({ user, startLevel }) {
  ...
  if (levelId) {
    ...
    // Next tier within the same topic (null once Master is cleared).
