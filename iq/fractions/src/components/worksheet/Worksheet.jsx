...
import { buildProblems } from '../../lib/fractionWorksheet'
...
export default function Worksheet({ topic, mode, onBack }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-rose-100 print:bg-white -mx-4 -my-6">
      ...
      <div className="no-print bg-white shadow p-4 flex justify-between items-center sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm font-semibold text-gray-600 cursor-pointer">
            <input type="checkbox" checked={showKey} onChange={(e) => setShowKey(e.target.checked)} className="w-4 h-4 accent-amber-500" />
            Answer key
          </label>
...
function Problem({ q, idx }) {
  ...
  if (q.layout === 'equation') {
    return (
      <div className="worksheet-problem flex items-center gap-3 border-b border-gray-200 pb-3">
        {n}
  ...
  if (q.layout === 'shade') {
    return (
      <div className="worksheet-problem flex items-center gap-4 border border-gray-400 rounded p-3">
        {n}
        <div className="flex-shrink-0"><FractionFigure fig={q.fig} bw size={110} /></div>
        <div>
          <p className="text-gray-700" style={{ fontSize: '12pt' }}>Color this much:</p>
          <p className="font-black" style={{ fontSize: '22pt' }}>{q.target}</p>
        </div>
      </div>
    )
  }

  // figcard (name / fill-the-whole)
  return (
