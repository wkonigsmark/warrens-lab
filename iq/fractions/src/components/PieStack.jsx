...
export default function PieStack({ num, den, size = 110 }) {
  const count = Math.max(1, Math.ceil(num / den))
  let left = num
  const pies = []
  for (let i = 0; i < count; i++) {
    const s = Math.min(den, Math.max(0, left))
    pies.push(<Pie key={i} parts={den} shaded={range(s)} size={size} />)
    left -= s
  }
  return <div className="flex flex-wrap justify-center items-center gap-3">{pies}</div>
}
