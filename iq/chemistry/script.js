...
function addToBeaker(symbol) {
    // No hard cap — beaker scales dynamically
    beakerContents.push(symbol);
    updateBeakerUI();
    
    // Quick "add" animation feedback
    const beaker = document.getElementById('beaker');
    beaker.classList.add('beaker-pulse');
    setTimeout(() => beaker.classList.remove('beaker-pulse'), 200);
}

function removeFromBeaker(symbol) {
    const index = beakerContents.lastIndexOf(symbol);
    if (index > -1) {
        beakerContents.splice(index, 1);
        updateBeakerUI();
    }
}
...
function updateBeakerUI() {
    ...
    const counts = {};
    beakerContents.forEach(sym => counts[sym] = (counts[sym] || 0) + 1);
    
    const displayItems = Object.entries(counts).map(([sym, count]) => ({
        symbol: sym,
        count: count
    }));
    ...
    // Dynamic tiers with mobile-specific scaling
    const isMobile = window.innerWidth <= 600;
    const scale = isMobile ? 0.82 : 1.0; // Shrink slightly for mobile beaker

    let atomSize, fontSize, gap, extra;
    if (displayCount <= 2)       { atomSize = 52 * scale; fontSize = isMobile ? '1rem' : '1.3rem';  gap = 12 * scale; extra = 12 * scale; }
    else if (displayCount <= 4)  { atomSize = 44 * scale; fontSize = isMobile ? '0.9rem' : '1.1rem'; gap = 10 * scale; extra = 10 * scale; }
    else if (displayCount <= 6)  { atomSize = 38 * scale; fontSize = isMobile ? '0.8rem' : '0.95rem'; gap = 8 * scale;  extra = 8 * scale; }
    else if (displayCount <= 10) { atomSize = 32 * scale; fontSize = isMobile ? '0.7rem' : '0.85rem'; gap = 6 * scale;  extra = 6 * scale; }
    else if (displayCount <= 16) { atomSize = 26 * scale; fontSize = isMobile ? '0.65rem' : '0.75rem'; gap = 5 * scale;  extra = 5 * scale; }
    else if (displayCount <= 25) { atomSize = 20 * scale; fontSize = isMobile ? '0.55rem' : '0.65rem'; gap = 4 * scale;  extra = 4 * scale; }
    else                         { atomSize = 16 * scale; fontSize = isMobile ? '0.45rem' : '0.55rem'; gap = 3 * scale;  extra = 3 * scale; }

    beakerDiv.style.gap = `${gap}px`;
    beakerDiv.style.padding = isMobile ? (displayCount > 8 ? '5px' : '10px') : (displayCount > 10 ? '5px' : '12px');
    beakerDiv.style.height = '100%';

    displayItems.forEach(item => {
        const el = document.createElement('div');
        el.className = 'beaker-atom bonded-atom';
        el.style.width  = `${atomSize + extra}px`;
        el.style.height = `${atomSize + extra}px`;
        el.style.fontSize = fontSize;
        el.title = `Click to remove 1x ${item.symbol}`;

        if (item.count > 1) {
            el.innerHTML = `${item.symbol}<sub style="font-size:0.75em">${item.count}</sub>`;
        } else {
            el.innerText = item.symbol;
        }

        el.addEventListener('click', () => removeFromBeaker(item.symbol));
        beakerDiv.appendChild(el);
    });

    const rawCount = beakerContents.length;
    const fillPercent = Math.min(95, (rawCount / 20) * 100); 
