/**
 * Ants & Atlases Geocode — UI controller for the Guess Who + Globle hybrid.
 *
 * Drives a single AAQuiz geocode session and renders:
 ...
 * The host page (script.js) gates clicks via AAGeocode.isActive().
 *
 * Public API:
 *   AAGeocode.init(opts)
 *   AAGeocode.start()
 *   AAGeocode.stop()
 *   AAGeocode.isActive()
 *   AAGeocode.handleMapClick(iso2, country)
 */
(function (global) {
  ...
  function buildChipDeck() {
    ...
    chips.innerHTML = "";
    for (const cat of AAQuiz.HINT_CATEGORIES) {
  ...
  function start(level = 1) {
    ...
    if (window.AACountries) AACountries.resetViewBox(document.getElementById("world-map"));
    startRound();
  }
  ...
  function dropBreadcrumb(result) {
    const feature = AACountries.getFeature(result.guess_iso2);
    if (!feature) return;
  ...
  function onRoundEnd(outcome) {
    const country = AACountries.getCountry(outcome.target.iso_a2);
  ...
  global.AAGeocode = { init, start, stop, isActive, handleMapClick };
})(window);
