/**
 * Ants & Atlases Mission — UI controller for the "Locate the country" quiz.
 *
 * Drives an AAQuiz session in 'mission' mode. Lighter than Geocode:
 ...
 *
 * Country clicks on the map route to AAMission.handleMapClick() when isActive() is true.
 *
 * Public API:
 *   AAMission.init()
 *   AAMission.start()
 *   AAMission.stop()
 *   AAMission.isActive()
 *   AAMission.handleMapClick(iso2, country)
 */
(function (global) {
  ...
  function start(level = 1) {
    ...
    if (window.AACountries) AACountries.resetViewBox(state.dom.svg);
  ...
  global.AAMission = { init, start, stop, isActive, handleMapClick };
})(window);
