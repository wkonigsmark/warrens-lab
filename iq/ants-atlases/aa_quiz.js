 ...
 *
 * Depends on window.AACountries — call AACountries.load() before starting a session.
 *
 * Usage:
 *   const session = AAQuiz.start({
 *     mode: "geocode",
...
(function (global) {
  ...
  const HINT_BUILDERS = {
    ...
    neighbor: c => {
      const picks = pickN(c.borders || [], 3)
        .map(iso3 => {
          const found = AACountries.allCountries().find(x => x.iso_a3 === iso3);
  ...
  function resolvePool(spec) {
    const all = AACountries.allCountries();
  ...
  function start(config) {
    ...
    if (!pool.length) throw new Error("AAQuiz: empty pool");
    ...
    function emit(name, ...args) {
      ...
      if (typeof h === "function") {
        try { h(...args, session); }
        catch (e) { console.error(`AAQuiz hook ${name} threw:`, e); }
      }
    }
    ...
    function guess(iso2) {
      ...
      const guessed = AACountries.getCountry(iso2);
