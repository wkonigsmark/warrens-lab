...
import { isConfigured, selectSessions, selectRoster, describeSession } from '../../../../_shared/progress/index.js'
...
// project directly and labels rows via the tool catalog. The roster itself
// comes live from the `student_roster` view, not a hardcoded list: add a
// row to `students` in Supabase and the new kid shows up here automatically.
export default function ProgressAdminView() {
  ...
  const load = () => {
    ...
    setRoster(null)
    setRows(null)
    Promise.all([selectRoster(), selectSessions('order=ts.desc&limit=500')])
      .then(([r, s]) => { setRoster(r); setRows(s) })
  ...
  const loading = roster === null || rows === null
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {error && <p className="text-red-500 mb-4">Couldn't load: {error}</p>}
      {loading && !error && <p className="text-gray-400">Loading…</p>}
      ...
      {!loading && (
        <>
          ...
          {students.length === 0 && (
            <p className="text-gray-400">No students in the roster yet — add one in Supabase's `students` table.</p>
          )}

          {students
            .filter((u) => studentFilter === 'all' || studentFilter === u.id)
            .map((u) => {
              const sessions = byStudent.get(u.id) ?? []
              const tools = perToolTotals.get(u.id)
              return (
