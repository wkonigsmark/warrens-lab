// page.js — the reusable lesson-page engine. Any instrument's lessons page is a
// three-line bootstrap: import this, import a pack, call initLessonsPage(). It
// renders a picker over the pack, draws each exercise on staves (reusing the
// composer's model/notation/audio modules), plays it back, transposes by
// key/instrument, prints a worksheet, and hands melodic exercises to Sing-Along.
//
// The page HTML must provide the standard element ids (see `els` below).
...
// control how it's written. Guitar sounds an octave below where it's written
// (staffShift +1, the little "8" under the clef); the xylophone sounds high.
// `tipKey` selects which per-exercise tip to show.
export const INSTRUMENTS = {
    ...
    xylo:    { label: 'Xylophone (C5–A6)',          octave: 0,  staffShift: 0,  clef: 'treble', tipKey: 'xylo' },
...
const TIP_ICON = { guitar: '🎸', piano: '🎹', xylo: '🎵' };

export function initLessonsPage(config) {
    const {
        lessons,
        defaultInstrument = 'guitar',
        // Note shown for exercises that can't be mic-scored (default: the chord case).
        noPracticeNote = 'Strummed chords can’t be mic-scored — the pitch tester hears one note at a time.',
    } = config;

    // --- Lesson-aware key options (labelled by each lesson's home) ----------
    function keyOptionsFor(L) {
        const base = tonicPc(L.home?.tonic || 'C');
        const mode = L.home?.mode || 'major';
        const opts = [];
    ...
    // --- Pack helpers over the passed-in lessons ----------------------------
    const lessonById = (id) => lessons.find((l) => l.id === id);
    const lessonNumber = (id) => lessons.findIndex((l) => l.id === id) + 1;
    const exerciseInLesson = (lessonId, exId) => (lessonById(lessonId)?.exercises || []).find((e) => e.id === exId);
    ...
    function tipFor(ex) {
        ...
        if (!text) return '';
        return `<p class="gl-tip"><span class="gl-tip-icon">${TIP_ICON[key] || '🎵'}</span>${text}</p>`;
    }
    ...
    function renderSections() {
        els.sections.innerHTML = exercises().map((ex, i) => {
            const practiceBtn = ex.practice
                ...
                : (noPracticeNote ? `<span class="gl-note">${noPracticeNote}</span>` : '');
            return `
    ...
    function buildManuscript() {
        ...
        return `<h2 class="ms-title">${lesson().icon} ${lesson().title} — ${keyLabel}</h2>`
            + `<p class="ms-by">${inst.label} · ♩ = ${state.tempo}</p>`
            + sections.join('');
    }
    ...
    // Chord-names toggle is optional (rhythm packs have no chords to name).
    if (els.chordNames) els.chordNames.addEventListener('click', () => {
