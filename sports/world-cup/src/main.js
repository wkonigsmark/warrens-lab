...
async function init() {
    ...
    document.getElementById('simulate-btn').onclick = () => {
        ...
        btn.innerHTML = '<span class="option-icon">⌛</span><span class="option-text">Simulating...</span>';

        setTimeout(() => {
            // Run simulation with current stochasticity
            simulatedResults = simulateMatches(allSchedule, teamIndices, currentStochasticity);
            const standings = calculateStandings(simulatedResults, nodes);
            groupStandings = standings.groupStandings;
            thirdPlaceStandings = standings.thirdPlaceStandings;
            ...
            alert("Simulation complete! Click 'Download Results' to save the data.");
        }, 50);
    };
